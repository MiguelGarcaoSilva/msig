#!/usr/bin/env python3
"""
STUMPY-based Case Study for Population Density Data
...
def load_population_density_data(data_path: str) -> Tuple[np.ndarray, pd.DataFrame]:
...
def compute_matrix_profiles(
    ...
    normalize: bool = True,
    include=None,
    results_dir: str = "../results/populationdensity"
) -> None:
...
def compute_motif_statistics_stumpy(
    ...
    r = np.ceil(EXCLUSION_ZONE_FACTOR * s) # r is the step size for non-trivial matches
    ...
    for motif_idx, (indices, distances, dimensions) in enumerate(zip(motif_indices, motif_distances, motif_subspaces)):
        ...
        assert max_allowed_dist is not None  
...
def main():
    ...
    results_dir = os.path.abspath(os.path.join(script_dir, "../../results/populationdensity/stumpy"))
