#!/usr/bin/env python3
"""
MOMENTI-based Case Study for Washing Machine Data
...
def load_washing_machine_data(csv_path: str) -> Tuple[np.ndarray, pd.DataFrame]:
    """
    Load washing machine sensor data and prepare for MOMENTI.
    
    Returns:
        X: Data transposed to (n_time, n_variables) for MOMENTI
        data_df: DataFrame with timestamps
    """
    logger.info(f"Loading washing machine data from {csv_path}")
    
    # Load CSV
    df = pd.read_csv(csv_path)
    
    # Select sensor columns (same as STUMPY: 6 sensors)
    sensor_cols = [
        'voltage', 'current', 'frequency',
        'power', 'power_factor', 'ambient_temperature'
    ]
    df = df[sensor_cols]
    ...
    logger.info(f"Sensors: {list(df.columns)}")
...
def compute_motif_statistics_momenti(
    ...
    """
    # Collect rows in a list first to avoid repeated concat calls (and the
    # FutureWarning pandas emits when concatenating empty/all-NA frames).
    stats_rows = []
    stats_columns = [
    ...
    for motif_data in motifs:
        ...
        # Store results as a simple dict in the list
        stats_row = {
            "ID": motif_id,
            "k": len(dimensions),
    ...
    # Build DataFrame once from the collected rows
    if len(stats_rows) == 0:
        return pd.DataFrame(columns=stats_columns)
    else:
        return pd.DataFrame(stats_rows, columns=stats_columns)
...
def main():
    ...
    results_dir = os.path.abspath(os.path.join(script_dir, "../results/washingmachine/momenti"))
    os.makedirs(results_dir, exist_ok=True)
    
    # Load data
    X, data_df = load_washing_machine_data(data_path)
    ...
    logger.info(f"  Corresponding to: {[f'{s/60:.1f}min' for s in subsequence_lengths]}")
    
    # MOMENTI parameters - OPTIMIZED FOR SPEED (Target: <2 hours/m)
    # Most significant motifs (low p-values) are still discovered
    k_motifs = 100  # Number of top motifs to return (maximize discovery)
    L = 50   # LSH repetitions (speed-optimized)
    K = 8    # LSH concatenations (optimal precision/speed balance)
    motif_dim_range = (2, 6)  # Search for motifs spanning 2-6 sensors
    
    logger.info(f"\nMOMENTI Parameters (Speed-Optimized for <2 hours/m):")
    ...
    for s in subsequence_lengths:
        logger.info(f"\n{'=' * 70}")
        logger.info(f"Processing motif length s={s} seconds ({s / 60:.1f} minutes)")
        logger.info(f"{'=' * 70}")
    ...
    # Save detailed results (ensure parent directory exists)
    os.makedirs(results_dir, exist_ok=True)
